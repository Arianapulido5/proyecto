...
export class OrderService {
  ...
  constructor() {
    ...
    if (savedHistory) {
      ...
      const lastOrder = parsedHistory.reduce(
        (max: number, order: Order) => (order.id > max ? order.id : max),
        0
      );
  ...
  private saveToLocalStorage() {
    ...
    localStorage.setItem(
      'orderHistory',
      JSON.stringify(this.historicalOrders.value)
    );
  }
  ...
  markOrderAsCompleted(orderId: number): void {
    ...
    this.historicalOrders.next(updatedOrders);
    this.saveToLocalStorage();
  }
}
